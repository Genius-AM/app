...
class QuadbikeNewOrder implements NewOrderMethod
{
    /**
     * @var array
     */
    ...
    public function setData(array $data)
    {
        ...
        $this->payload['amount'] = $this->data['men'] + $this->data['women'] + $this->data['kids'];
        $this->payload['old_amount_people'] = 0;

        $this->company = Company::findOrFail($this->data['company_id']);

        $this->car = $this->company->driver->cars->first();
    }
    ...
    public function addOrder()
    {
        ...
        if (!empty($our_excursion) && !empty($excursions) && count($excursions) >= (int)$raceNumber) {
            ...
            $can_seat_men = $pass_amount['men'] ? $pass_amount['men'] - $amount['men'] : 5 - $amount['men'];
            if ($can_seat_men < $this->data['men']) {
                return response(['message'=>"Заявка не добавлена. На данное время свободных мест для мужчин: {$can_seat_men}"], 400);
            }
            $can_seat_women = $pass_amount['women'] ? $pass_amount['women'] - $amount['women'] : 5 - $amount['women'];
            if ($can_seat_women < $this->data['women']) {
                return response(['message'=>"Заявка не добавлена. На данное время свободных мест для женщин: {$can_seat_women}"], 400);
            }
            $can_seat_kids = $pass_amount['kids'] ? $pass_amount['kids'] - $amount['kids'] : 5 - $amount['kids'];
            ...
            $capacity = !empty($this->pass_amount) ? $this->pass_amount->getAmount() : 15;
            ...
            if ($pass_amount['men'] < $this->data['men']) {
                return response(['message'=>"Заявка не добавлена. На данное время свободных мест для мужчин: {$pass_amount['men']}"], 400);
            }
            if ($pass_amount['women'] < $this->data['women']) {
                return response(['message'=>"Заявка не добавлена. На данное время свободных мест для женщин: {$pass_amount['women']}"], 400);
            }
    ...
    public function editOrder()
    {
        ...
        if (!empty($excursion)) {
            ...
            $can_seat_men = $pass_amount['men'] ? $pass_amount['men'] : 5 - $amount['men'];
            if ($can_seat_men < $this->data['men']) {
                return response(['message'=>"Заявка не изменена. На данное время свободных мест для мужчин: {$can_seat_men}"], 400);
            }
            $can_seat_women = $pass_amount['women'] ? $pass_amount['women'] : 5 - $amount['women'];
            if ($can_seat_women < $this->data['women']) {
                return response(['message'=>"Заявка не изменена. На данное время свободных мест для женщин: {$can_seat_women}"], 400);
            }
            $can_seat_kids = $pass_amount['kids'] ? $pass_amount['kids'] : 5 - $amount['kids'];
            if ($can_seat_kids < $this->data['kids']) {
    ...
    private function editOrderData(Order $order, Client $client, Route $route)
    {
        ...
        $order->driver_id = $this->company->driver->id;
    ...
    private function detachExcursionDoNotMatchDataTime(Order $order)
    {
        if (count($order->excursion) > 0) {
            $this->payload['old_amount_people'] = (int) $order->men + (int) $order->women + (int) $order->kids;
