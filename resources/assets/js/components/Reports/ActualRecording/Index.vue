<template>
  <div>
    ...
    <div class="row">
      ...
      <div class="col-md-6" v-if="report.length">
    ...
    <div class="row category-filter mb-2" v-for="category in selectedCategories">
      <div class="col-md-2 d-flex align-items-center font-weight-bolder">
        {{ category.name }}
      </div>
      <div class="col-md-5">
        ...
        <multiselect
          :multiple="true"
          v-model="selectedManagers[category.id]"
          :options="managers"
          label="name"
          track-by="id"
          placeholder="Выберите из списка"
      ...
      <div class="col-md-5" v-if="category.subcategories.length > 1">
      ...
      <div class="col-md-5" v-if="category.id === 3">
    ...
    <ActualRecordingBaseTable
      v-if="report.length && !selectedDays && !selectedHours"
      :categories="selectedCategories"
      :report="report"
    />

    <ActualRecordingDayTable
      v-if="report.length && selectedDays && !selectedHours"
      :categories="selectedCategories"
      :report="report"
    />

    <ActualRecordingHourTable
      v-if="report.length && selectedHours"
      :categories="selectedCategories"
      :report="report"
    />
  </div>
...
import ActualRecordingBaseTable from './BaseTable.vue'
import ActualRecordingDayTable from './DayTable.vue'
import ActualRecordingHourTable from './HourTable.vue'
...
export default {
    ...
    selectedCategories: {
      handler: function () {
        ...
        for (const category of this.selectedCategories) {
          ...
          if (this.selectedManagers.hasOwnProperty(category.id)) {
            managers[category.id] = this.selectedManagers[category.id];
          }
  ...
  computed: {
    queryData: function () {
      return {
        ...
        managers: this.getObjId(this.selectedManagers),
  ...
  methods: {
    ...
    loadExcel() {
      ...
      axios
        .post(
          '/reports/actual-recording/excel',
          {
            report: this.report,
        ...
        .then(response => {
          ...
          link.href = url;
          link.setAttribute('download', 'actual-recording.xlsx');
