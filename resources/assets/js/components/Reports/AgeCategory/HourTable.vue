    ...
    <table>
      <thead>
        <tr>
          ...
          <th :colspan="days.length * hours.length" v-for="ageCategory in ageCategories">{{ ageCategory.name }}</th>
      ...
      <tbody>
          ...
          <template v-for="ageCategory in ageCategories">
            <template v-for="day in days">
              <td v-for="hour in hours">
                {{ getManagerHourValue(manager, ageCategory, day, hour) }}
              </td>
            </template>
          </template>
          <td>
        ...
        <tr class="border-top">
          ...
          <template v-for="ageCategory in ageCategories">
            <template v-for="day in days">
              <td v-for="hour in hours">
                {{ getCategoryHourSumma(ageCategory, day, hour) }}
              </td>
            </template>
          </template>
          <td>
...
export default {
  ...
  computed: {
    ...
    hours: function () {
      if (Object.keys(this.report.categories).length !== 0) {
        return Object.keys(Object.values(this.report.categories[0].data.managers[0].dates)[0].times);
      }
      ...
      for (const categoryElement of this.report.categories) {
        for (const managerElement of categoryElement.data.managers) {
          if (managerElement.id === manager.id) {
            value += managerElement.dates[day].times[hour].ages[ageCategory.id];
            break;
          }
        }
      }
